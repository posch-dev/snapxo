...
def test_a_broken_entry_costs_only_that_file(tmp_path):
    ...
    # Corrupt the stored bytes so the CRC no longer matches what the header promises
...
# The bomb check works on the two numbers from the central directory, so it can be
# tested without ever building an archive that would fill the disk.
