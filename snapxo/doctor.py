# `snapxo doctor`: what is installed, what is missing, and how to fix it.
...
def run_doctor() -> bool:
    # Returns False when something needed for a normal run is absent.
    ...
    if not has_ffmpeg:
        ...
        console.print("    [dim]or run with --no-encode --no-overlay[/dim]")
    if not browser_ok:
        console.print("[bold]The PDF formats need Chromium:[/bold]")
        if reason == "package":
            console.print("    [cyan]pip install playwright[/cyan]")
        console.print("    [cyan]playwright install chromium[/cyan]")
        console.print("    [dim]or stay on the html formats, which need nothing[/dim]")
    return False
