# Paths are relative, so a folder can move between drives and between systems.
...
def build_manifest(
    ...
    for i, entry in enumerate(file_index):
        ...
        entries.append({
            ...
            # Includes the IDs whose own copy dedup removed.
...
def write_manifest(
    ...
    data = build_manifest(output_dir, file_index, own_username, sources, timezone_name)
...
def build_media_id_map(
    ...
    # dup_alias maps a path dedup deleted to the one it duplicated. Without it
    # those messages would show no media at all.
