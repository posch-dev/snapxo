# Two exports of one account overlap almost completely and each numbers its year
# folders from 0001, so merging is: dedupe by content, renumber chronologically,
# rebuild the pages. Media is copied or hardlinked, never re-encoded.
...
from ..filetypes import AUDIO_EXTS, IMAGE_EXTS, VIDEO_EXTS
from ..media.metadata import apply_file_times
...
def _deduplicate(
    ...
    # The hash is needed anyway, so checking it against the checksums is free.
    seen: dict[str, dict] = {}
    unique = []
    dropped = 0
    freed = 0
...
def _recover_damaged(records: list[dict]) -> tuple[list[dict], int]:
    # A file damaged in one export may be intact in another, which is the point.
...
def _transfer(src: Path, dst: Path, hardlink: bool) -> str:
    ...
    if hardlink:
        try:
            ...
            # Different filesystem, or the OS refused.
            pass
...
def export_recency(input_dir: Path) -> str:
    # Empty without a manifest, which then sorts as the oldest.
    manifest = load_manifest(input_dir)
    return str(manifest.get("generated") or "") if isinstance(manifest, dict) else ""


def merge_json_data(datas: list[dict]) -> dict:
    # Pass the newest export first: a plain value keeps the first one seen, so the
    # order decides whose Snapscore survives.
    merged: dict = {}
    for data in datas:
...
def expand_inputs(inputs: list[Path]) -> list[Path]:
    # A parent folder of archives beats listing a dozen of them by hand.
    resolved: list[Path] = []
    seen: set[Path] = set()
...
def _verify_transferred(records: list[dict], output: Path) -> list[str]:
    # Re-hashed before any source folder is deleted.
    problems = []
    with Progress(console=console) as progress:
...
def merge_outputs(
    ...
    for input_dir in inputs:
        ...
        if data:
            dated_datas.append((export_recency(input_dir), data))
    ...
    # Newest first, so a single value like Snapscore comes from the newer export.
    dated_datas.sort(key=lambda pair: pair[0], reverse=True)
    json_data = merge_json_data([data for _, data in dated_datas]) if dated_datas else {}
