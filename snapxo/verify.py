# Checks a finished output folder against its manifest, and against the checksums
# of an earlier run if the folder has them.
...
from .utils import MEDIA_EXTS, file_hash
...
def verify_folder(folder: Path, hashes: bool = False) -> tuple[Report, dict[str, str]]:
    # Returns the report and the hashes computed along the way, empty unless
    # `hashes` is set. Without it only existence and size are checked, which is
    # instant and still finds the common half-copied folder.
