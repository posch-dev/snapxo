...
def find_export_inputs(inputs: list[Path]) -> tuple[list[Path], Path | None]:
    ...
    for inp in inputs:
        if inp.is_file() and inp.suffix.lower() == ".zip":
            # Named explicitly, so validation complains later rather than here.
            zips.append(inp)
        elif inp.is_dir():
            # A directory like Downloads turns up unrelated ZIPs.
...
# SnapXO's own bookkeeping, which sits in _meta beside the export JSON without
# being part of it. Read as export data, --no-meta folders look like they have some.
OWN_JSON_FILES = {"manifest.json", "checksums.json", "integrity.json"}
...
def load_json_data(export_dir: Path) -> dict:
    ...
    if json_dir.is_dir():
        search_dir = json_dir
    elif any(f.name not in OWN_JSON_FILES for f in export_dir.glob("*.json")):
        search_dir = export_dir
    else:
        return data

    for f in search_dir.iterdir():
        if f.suffix.lower() == ".json" and f.name not in OWN_JSON_FILES:
            try:
...
def count_json_stats(json_data: dict) -> dict:
    # Every key and wrapper below is the export format as of August 2026.
    stats = {}

    # chat_history keys are usernames or group UUIDs.
