...
# A real export is compressed media and sits near 1x. Both limits must be
...
def free_space(path: Path) -> int:
    # The target may not exist yet, so measure the nearest existing parent.
    probe = path
...
def _is_unsafe_name(name: str) -> str | None:
    # Both path flavours, because a Windows-written ZIP can carry a `C:\` that
    # PurePosixPath reads as an ordinary file name.
...
def safe_extract(zip_path: Path, dest: Path, verbose: bool = False) -> tuple[int, list[dict]]:
    # A broken entry costs that file, not the run.
