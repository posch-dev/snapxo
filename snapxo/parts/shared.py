# The pages open from disk, so no fetch, no modules, no external assets.

DATE_FORMATS = [
    ("iso", "YYYY-MM-DD"),
    ("dmy", "DD.MM.YYYY"),
    ("mdy", "MM/DD/YYYY"),
    ("dmonthy", "DD Month YYYY"),
    ("monthdy", "Month DD, YYYY"),
]
...
def date_format_picker_html(label: str = "Date formatting") -> str:
    options = "".join(
...
def date_span(iso_date: str, time: str = "", extra_class: str = "", suffix: str = "") -> str:
    # Renders ISO when the script never runs.
    if not iso_date:
        return ""
...
def split_timestamp(value: str) -> tuple[str, str]:
    # ("", "") for anything not ISO-ish, so the caller can print the raw value.
