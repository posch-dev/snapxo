...
def _run(cmd: list[str], **kwargs):
    # Attached stdin lets ffmpeg swallow the answer to the next prompt.
    return subprocess.run(cmd, stdin=subprocess.DEVNULL, **kwargs)


class FFmpeg:
    def __init__(self, ffmpeg_path: str = "ffmpeg", ffprobe_path: str = "ffprobe",
                 software_encoding: bool = False, crf: int = 23):
        self.ffmpeg = self._resolve(ffmpeg_path, "ffmpeg")
        self.ffprobe = self._resolve(ffprobe_path, "ffprobe")
        self.crf = crf
        # Without ffmpeg the "falling back to libx265" note would be misleading.
        self.qsv_available = False
        if not software_encoding and self.check():
    ...
    def _resolve(self, path: str, name: str) -> str:
        ...
        if resolved:
            return resolved
        if path != name:
            # Said out loud rather than silently replaced by whatever is on PATH.
    ...
    def _static_binaries() -> dict[str, str] | None:
        # static-ffmpeg builds lack QSV, so a system ffmpeg stays the better one.
    ...
    def _detect_qsv(self) -> bool:
        ...
        # A real encode at a realistic resolution, not a capability list.
        try:
            result = _run(
    ...
    def probe(self, filepath: Path) -> dict:
        try:
            result = _run(
                [self.ffprobe, "-v", "error", "-show_entries",
                 "format=duration,bit_rate:stream=codec_name,codec_type,width,height",
        ...
        picked = video or audio or {}

        info = {}
        if picked.get("codec_name"):
            info["codec"] = picked["codec_name"]
        if video and video.get("width") and video.get("height"):
            info["width"] = int(video["width"])
            info["height"] = int(video["height"])
        try:
            info["duration"] = round(float(container["duration"]), 1)
        except (KeyError, TypeError, ValueError):
            pass
        try:
            info["bitrate"] = int(container["bit_rate"])
        except (KeyError, TypeError, ValueError):
            pass
        return info
    ...
    def grab_frame(self, input_path: Path, output_path: Path, height: int = 320) -> bool:
        # A second in, the opening frame of a Snapchat video is often black.
        try:
            cmd = [
                self.ffmpeg, "-y", "-ss", "1",
                "-i", str(input_path),
