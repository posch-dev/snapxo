...
def find_duplicates(files: list[Path]) -> list[tuple[Path, list[Path]]]:
    # Find duplicates by MD5. Returns [(keep, [duplicates])].
...
def remove_duplicates(files: list[Path], dry_run: bool = False,
    ...
    # Returns (removed_count, freed_bytes, alias). `alias` maps each removed path to
    # the one it duplicated, so callers can still resolve media that is now gone.
...
def collect_media_files(directory: Path) -> list[Path]:
    # Collect all media files from a directory tree.
    files = []
